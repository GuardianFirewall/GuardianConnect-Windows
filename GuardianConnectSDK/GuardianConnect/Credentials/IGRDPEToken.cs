...
namespace GuardianConnect.Credentials
{
    public interface IGRDPEToken
    {
        ...
        static GRDPEToken InitFromDictionary(Dictionary<string, object> dict)
        {
            ...
            if (dict.ContainsKey("Token")) peToken.Token =
 dict["Token"].ToString() ?? throw new InvalidOperationException();
            if (dict.ContainsKey("expirationDateUnix")) peToken.ExpirationDateUnix = (long)dict["ExpirationDateUnix"];
            if (dict.ContainsKey("ExpirationDate")) peToken.ExpirationDate =
 DateTime.Parse(dict["ExpirationDate"].ToString() ?? throw new InvalidOperationException());
            if (dict.ContainsKey("ConnectAPIEnv")) peToken.ConnectAPIEnv =
 dict["ConnectAPIEnv"]?.ToString() ?? Common.DefaultConnectAPIHostname;
            if (dict.ContainsKey("SubscriptionType")) peToken.SubscriptionType =
 dict["SubscriptionType"].ToString() ?? throw new InvalidOperationException();
            if (dict.ContainsKey("SubscriptionTypePretty")) peToken.SubscriptionTypePretty =
